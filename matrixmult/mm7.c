...
// gcc -g -Wall -o mm7 mm7.c -pthread -lrt
...
void op_mat(float *);
void mult(void);
double elpase_time(struct timeval *, struct timeval *);
...
void mult(void)
{
                ...
                for (j = 0; j < dim; ++j){
                    for (k = 0; k < dim; ++k){
                        ELEMENT(result, i, j, dim) +=
                            ELEMENT(matrix1, i, k, dim)
                            * ELEMENT(matrix2, k, j, dim);
                    }
                }
            }
    ...
    fprintf(stderr, "all child process cleand up\n");
}
